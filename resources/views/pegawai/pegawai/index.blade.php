...
<div class="container mx-auto px-4 py-8 space-y-6">
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Manajemen data pegawai perusahaan</p>
    </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="relative w-full max-w-lg bg-white rounded-lg shadow-xl">
            <div class="flex justify-between items-center px-5 py-3 border-b border-gray-200">
                <h3 id="modalTitle" class="text-base font-semibold text-gray-900">Tambah Pegawai</h3>
            ...
            <form id="pegawaiForm" enctype="multipart/form-data" class="p-5">
                @csrf
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="relative w-full max-w-sm bg-white rounded-lg shadow-xl p-5">
            <div class="text-center">
                ...
                <p class="text-xs text-gray-600 mb-4">Apakah Anda yakin ingin menghapus pegawai ini?</p>
    ...
    pegawai.forEach(item => {
        const row = `
            <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                ...
                <td class="px-6 py-3">
                    <span class="px-3 py-1 inline-flex text-xs font-semibold rounded-full ${item.role === 'admin' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}">
                ...
                <td class="px-6 py-3">
                    <div class="flex items-center justify-end gap-2">
                        <button onclick="viewPegawai(${item.id_pegawai})" class="p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-md transition-all" title="Lihat Detail">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                            </svg>
                        </button>
                        <button onclick="editPegawai(${item.id_pegawai})" class="p-1.5 bg-green-50 text-green-600 hover:bg-green-100 rounded-md transition-all" title="Edit">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                            </svg>
                        </button>
                        <button onclick="deletePegawai(${item.id_pegawai})" class="p-1.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-md transition-all" title="Hapus">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function renderPagination(pagination) {
    ...
    for (let i = Math.max(1, pagination.current_page - 2); i <= Math.min(pagination.last_page, pagination.current_page + 2); i++) {
        if (i === pagination.current_page) {
            ...
            html += `<button onclick="loadPegawai(${i})" class="px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-200 rounded hover:bg-gray-50 transition-all">${i}</button>`;
        }
    }
...
function editPegawai(id) {
    ...
    .then(data => {
        ...
        document.getElementById('pegawaiId').value = data.id_pegawai;
